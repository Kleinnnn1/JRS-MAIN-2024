...
async function getDeptHeadDeclinedJobRequest() {
  try {
    ...
    const { data, error } = await supabase
      ...
      .eq("status", "declined") // Filter by status = 'declined'
...
export default function JobDeclinedContent() {
  ...
  useEffect(() => {
    async function fetchRequests() {
      try {
        setLoading(true);
        const data = await getDeptHeadDeclinedJobRequest();
        setRequests(data);
        setError(null);
      } catch (err) {
        console.error("Error fetching declined job requests:", err);
  ...
  return (
    <div className="my-4 mx-3 py-2 px-4 bg-white shadow-lg rounded-lg">
      <div className=" py-4 px-6 bg-custom-blue py-2 px-4 flex justify-between items-center rounded-t-lg">
        ...
        <SearchBar title="Declined" showInput={true} />
