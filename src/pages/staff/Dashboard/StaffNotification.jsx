...
export default function StaffNotification() {
  ...
  const checkNewRequests = async () => {
    ...
    if (newRequests.length > 0) {
      // Insert notification for the requestor
  ...
  const checkRequestStatusUpdates = async () => {
    ...
    // Loop through requests and check if status is "Ongoing" or "Completed" or extended
  ...
  // NEW: Check for due dates within the next day and insert reminder notifications
  const checkDueDateReminders = async () => {
    ...
    // Fetch ongoing requests that are due within the next 24 hours
    const { data: dueRequests, error: dueError } = await supabase
      .from("Request")
      .select("*")
      .eq("idNumber", userIdNumber)
      .eq("status", "Ongoing") 
      .gt("expectedDueDate", now.toISOString())
      .lt("expectedDueDate", oneDayFromNow.toISOString());

    if (dueError) {
      console.error("Error fetching due requests:", dueError);
      return;
    }
    ...
    for (const request of dueRequests) {
      const message = `REMINDER: Your due date is near for Job Request No. ${request.requestId}.`;
  ...
  // Poll every 1 second to check for new job requests, status updates, due date reminders, and refresh notifications
  useEffect(() => {
    const interval = setInterval(() => {
      checkNewRequests(); // Check for new requests and insert notifications
      checkRequestStatusUpdates(); // Check for status changes and insert notifications
      checkDueDateReminders(); // Check for due dates and insert reminder notifications
