...
export default function SysAdminAddNewSpme() {
  
  const [SpmeFirstName, setSpmeFirstName] = useState('');
  const [SpmeLastName, setSpmeLastName] = useState('');
  ...
  const handleSubmit = (event) => {
    ...
    Swal.fire('Success', 'Successfully Added New Spme', 'success');
  ...
  const handleCancel = (event) => {
    event.preventDefault();
    Swal.fire('Cancelled', 'Spme addition was cancelled', 'error');
  };

  return (
    <div className="m-16 bg-white shadow-md rounded-lg">
      <div className="bg-yellow-500 p-5 rounded-t-lg">
        <p className="text-xl font-bold text-gray-600">New Spme Information</p>
      </div>
      <form onSubmit={handleSubmit}>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 p-5">
          <div>
            <label className="block text-gray-700 font-bold mb-2" htmlFor="SpmeFirstName">
              First Name
            </label>
            <input
              type="text"
              ...
              value={SpmeFirstName}
              onChange={(e) => setSpmeFirstName(e.target.value)}
              required
            />
          </div>
          <div>
            <label className="block text-gray-700 font-bold mb-2" htmlFor="SpmeLastName">
              Last Name
            </label>
            <input
              type="text"
              ...
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500"
              value={SpmeLastName}
              onChange={(e) => setSpmeLastName(e.target.value)}
              required
            />
          </div>
          <div>
